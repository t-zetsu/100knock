...
def gradient(X,y,W):
    grad = torch.tensor([[0 for j in range(n_dim)] for i in range(n_label)],dtype=float).to(device) #勾配の初期化
    y_list = y.tolist()
    n = len(y_list)
    P = torch.nn.functional.softmax(torch.matmul(X,W),1) #予測行列
    T = torch.tensor([[1 if j == y_list[i] else 0 for j in range(n_label)] for i in range(n)]).to(device) #正解行列
    for i in range(n):
...
def save_checkpoints(W,m,v,epoch):
    torch.save(W,f'checkpoints/epoch{epoch}_W.pt')
    torch.save(torch.stack([m,v]),f'checkpoints/epoch{epoch}_state.pt')
...
def SGD(X_train,X_valid,y_train,y_valid,W):
    ...
    for epoch in range(n_epoch):
        ...
        s_time = time.time()
        while k+batch_size <= X_train.size()[0]:
            X = torch.cat([X_train[i:i+1] for i in shuffle_list[k:k+batch_size]]).to(device) #batchごと
            y = torch.cat([y_train[i:i+1] for i in shuffle_list[k:k+batch_size]]).to(device)
        ...
        e_time = time.time()
        print(f'epoch {epoch+1} : loss_train {loss_train}, accuracy_train {accuracy_train}, loss_valid {loss_valid}, accuracy_valid {accuracy_valid}, {(e_time - s_time):.4f}sec')
...
X_valid = torch.load('data/X_valid.pt').to(device)
y_valid = torch.load('data/y_valid.pt').to(device)
...
for batch_size in [1,2,4,8,16,32,64,128,256,512,1024]: 
    ...
    W = SGD(X_train,X_valid,y_train,y_valid,W = torch.zeros(n_dim, n_label,device=device,dtype=torch.float))
